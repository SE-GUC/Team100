...
router.delete("/:id", passport.authenticate('jwt', { session: false }), async (req, res) => {
  if (req.user.user_type === "mun_admin") {
    try {
      ...
      if (!messageId) return res.status(404).send({ error: "Message does not exist" });
      if (deletedMessages.committee !== req.user.committee_type) return res.status(404).send({ error: "Unauthorized" });
      else {
        res.json({
    ...
    try {
      ...
      if (isValidated.error)
        return res
          .status(400)
          .send({ error: isValidated.error.details[0].message });
      Message.updateOne({ _id: id }, { $set: req.body })
        .exec()
        .then(() => {
